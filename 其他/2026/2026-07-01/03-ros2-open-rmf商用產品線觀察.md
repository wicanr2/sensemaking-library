...
### 補充影片：Dexory × Neele-Vat 的商用場域訊號

使用者補充的 Dexory 官方 YouTube 影片《Dexory Robot x Neele Vat | Autonomous Robots Transform Inventory Management》（2026-01-30）提供了一個更貼近商業場域的證據：Dexory 描述其 autonomous robots 自 2025 年初在 Neele-Vat 位於 Rotterdam Pernis 的倉儲場域運作，覆蓋約 40,000 個 pallet locations，並將整倉掃描自動化到每週 3 次。

影片與描述中值得記錄的商業訊號是：

- robot 每晚在倉儲中自主巡行，從 wall to wall、floor to ceiling 掃描儲位。
- 目標不是「展示機器人會走」，而是把 cycle counting 從人工週期盤點變成連續資料採集。
- 影片描述 robot 可掃描最高約 14m 貨架，速度可達每小時約 10,000 個 pallet locations。
- AI / computer vision 將原始 physical data 轉成 DexoryView / Dexory 軟體平台中的 actionable insights。
- 客戶端價值集中在 misplaced pallets、unexpected empty locations、stock accuracy、root-cause analysis 與服務可靠度。

這段影片不能直接證明 Dexory 使用 Open-RMF；它比較強化的是「ROS 2 / Nav2 類 runtime 的商業需求為何重要」：倉儲客戶真正付錢的是庫存可視性與盤點可靠度，而不是底層 middleware 名稱。
